...
const chkStayOnPage = document.querySelector("form #stayOnThisPage");
...
form.addEventListener("submit", (e) => {
  ...
  if (!chkAccept.checked) {
    ...
    HTML.appendError(
      "\nYou have to agree that every data is correct",
      chkAccept
    );
    return;
  }
...
function submitFormData(data, url) {
  axios.post(url, data).then((response) => {
    if (response.data.dataStatus === "success") {
      if (chkStayOnPage.checked) {
        form.reset();
        resultDiv.innerHTML = displayAlert(response.data.message).outerHTML;
      } else window.location = `/admin/users/?coach-registered-successful`;
